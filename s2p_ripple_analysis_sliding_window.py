...
def sliding_peak_to_peak(
    ...
    window of width window_hz. The window is a rigid n_pts-wide block that
    shifts as a unit — it never shrinks — when it approaches either edge.

    Edge handling — the window is always exactly n_pts wide and shifts as a
    rigid unit rather than shrinking when it approaches either edge:

      - i=0 (f_min)                : pure forward  [f_min, f_min + window]
      - 0 < i < half               : forward-biased, window still anchored left
      - half <= i <= n - half - 1  : freely centred around i
      - n - half <= i < n - 1      : backward-biased, window anchored right
      - i=n-1 (f_max)              : pure backward [f_max - window, f_max]

    With a 250 MHz window and 50 MHz step (n_pts = 6, half = 3):
      i=0 : [0, 250] MHz  — 0 left,   250 right
      i=1 : [0, 250] MHz  — 50 left,  200 right
      i=2 : [0, 250] MHz  — 100 left, 150 right
      i=3 : [0, 250] MHz  — 150 left, 100 right  (transition; 150 MHz > 125 MHz half-window)
      i=4+: window floats freely, centred on i
      ...mirrors in reverse as i approaches n-1.
    ...
    """
    n_pts = int(round(window_hz / freq_step_hz)) + 1   # window width in samples
    half  = n_pts // 2                                  # samples to left of centre
    n     = len(s21_db)
    ptp   = np.empty(n)

    for i in range(n):
        start = i - half
        end   = start + n_pts      # always exactly n_pts wide

        # Shift the whole window right if it overruns the left edge
        if start < 0:
            start = 0
            end   = n_pts

        # Shift the whole window left if it overruns the right edge
        if end > n:
            end   = n
            start = n - n_pts
...
def main() -> None:
    ...
    print(f"\n  Window size     : {args.window_mhz:.0f} MHz  (±{int(round(window_hz/freq_step_hz))//2} points from centre)")
    if valid_ptp.size:
        peak_idx = np.nanargmax(ptp)
