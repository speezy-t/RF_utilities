...
def sliding_peak_to_peak(
    ...
    [f_i, f_i + window_hz) and compute max(S21_dB) - min(S21_dB).

    The window is *forward-looking* from each point, matching the
    description "beginning with the first frequency point".

    Returns
    -------
    ptp : np.ndarray, same length as frequencies
        Peak-to-peak ripple in dB at each frequency point.
        Points near the end where a full window cannot be formed are set to NaN.
    """
    n_pts_in_window = int(round(window_hz / freq_step_hz)) + 1  # inclusive
    n = len(s21_db)
    ptp = np.full(n, np.nan)

    for i in range(n):
        end_idx = i + n_pts_in_window          # exclusive upper bound
        window_data = s21_db[i:end_idx]        # slices to end of array naturally
